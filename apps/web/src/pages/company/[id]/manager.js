...
export async function getServerSideProps({ req, params }) {
  ...
  try {
    ...
    // доступ сюда: manager / org_department / admin / owner
    if (!(role === 'manager' || role === 'org_department' || role === 'admin' || role === 'owner')) {
...
export default function CompanyManagerPage({ company, role }) {
  const label =
    role === 'org_department' ? 'manager (org_department)' :
    role === 'admin' ? 'admin' :
    role === 'owner' ? 'owner' :
    'manager'
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 flex flex-col">
      {/* Хедер — тот же, что у тебя в [id].js */}
