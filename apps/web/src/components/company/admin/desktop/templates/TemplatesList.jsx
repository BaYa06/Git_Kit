function TemplateRow({ template, selected, onSelect }) {
  const durationLabel =
    template.durationDays || template.durationDays === 0
      ? `${template.durationDays} ${template.durationDays === 1 ? 'день' : 'дней'} / ${template.nights ?? 0} ночей`
      : '—';
  ...
  return (
    ...
    >
      <div className="col-span-4 flex flex-col gap-0.5">
        <span className={`text-sm font-bold ${selected ? 'text-white' : 'text-gray-200 group-hover:text-white'} transition-colors`}>
          {template.name}
        </span>
      ...
      <div className="col-span-1 text-center">
        <span className="text-sm text-gray-300">{template.nights ?? '—'}</span>
      </div>
      <div className="col-span-2">
    ...
    <div className="xl:col-span-8 flex flex-col gap-4">
      <div className="glass-card rounded-2xl overflow-hidden flex flex-col min-h-[500px]">
        <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/10 bg-white/5 text-xs font-semibold text-gray-400 uppercase tracking-wider">
          ...
          <div className="col-span-1 text-center">Ночи</div>
